...
# get Alertmanager app name from current cluster
app=$(tsh apps ls --format yaml --query='labels["cluster"] == "'"$context"'" && labels["application"] == "alertmanager"' | yq '.[].metadata.name')

# check if app is defined
if [[ -z "${app}" ]]; then
  exit_err "error: Alertmanager app not found"
fi
...
amtool --alertmanager.url=http://localhost:"$port" "$@"
